<template>
    ...
    <v-row>
        <v-col v-for="item in items" :key="item.name_invite" cols="12" sm="6" md="6" lg="3">
            <v-card>
...
<script>
    export default {
        ...
        computed:{
            ...
            items(){
                return this.$store.state.share.items;
            }
        },
        methods: {
            remove_item:async function(item){
                await this.$store.dispatch('share/remove-member',item.id);
            },
            close() {
                this.add_user = false;
                setTimeout(() => {
