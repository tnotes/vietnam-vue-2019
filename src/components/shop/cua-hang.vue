    ...
    <v-card>
        ...
        >
            ...
            <template v-slot:item.shop_name="{ item }">
                <v-chip  :href="item.url" target="_blank" :color="getColor(item.ecom_name)"   label dark>{{ item.shop_name }}</v-chip>
            </template>
    ...
    export default {
        data: () => ({
            ...
            headers: [
                ...
                {text: 'Cửa hàng của tôi', value: 'shop_name'},
        ...
        methods: {
            ...
            deleteItem(item) {
                confirm('Bạn có chắc chắn muốn xóa cửa hàng này khỏi danh sách ?') && this.$store.dispatch('shop/delete-shop', item);
            },
